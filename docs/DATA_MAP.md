...
| `reconcile_targets` | the PUT handler, only when `scope_text` is present | rebuilds `scope_targets` from the submitted `scope_text`, and rejects the change with 422 `referenced_scope_removed` if it would strand a finding, including a `custom` finding whose only locations were typed under a dropped app type |
...
**A library-inserted finding starts with `scope.mode == "custom"` and no targets**, not the `Scope` default of `"all"`. `insert_library` passes `Scope(mode="custom")` explicitly, because `"all"` would read as a deliberate every-target choice and carry the new finding straight past the affected-location gate (`finding_is_complete`) before the tester has picked anything. A finding created this way is therefore incomplete by design until a location is chosen.
...
| scope resolves to a location | `scope_has_location`, `_scope_reaches_a_location` | `scopeHasLocation`, `scopeTargetIds`, `scopeReaches` |
