...
`pendingLibraryOffers` is the **single owner** of "which sections still have an unanswered offer" — the Content-page banners and the readiness panel both read it, so they can never disagree. Beyond `content_offer_resolved`, it suppresses any section whose current fragments already equal the library's (`sameFragments`, which compares `JSON.stringify` with `frag_id` stripped, since reminting guarantees the ids differ on every copy). That equality check is what keeps drafts written before `insert_library` started pre-answering the field from offering content they already hold; it also means an offer reappears if the tester later empties or rewrites such a section, which is the honest outcome — the offer would now actually change something.
