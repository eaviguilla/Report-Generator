...
| setup completeness | `setup_issues` / `setup_is_complete` | `validateSetupPage`, `updateSetupValidationNotice` — **and the count is taken from the DOM, not the model**: both read every `textarea` inside a `#scope-grid .scope-panel`, as do `setupSectionSummary` and the scope textarea's own `oninput`. Four sites. Any change that puts a second textarea in a scope panel has to narrow all four, or a box that is not a scope target passes the client gate and 422s on the server |
...
| canonical app-type order | `CHANNELS` in `models.py`; `docx_report.CHANNEL_ORDER` is a `list()` copy of it | `CHANNELS`, module scope in `app.js`, **and a third copy** — `variants` in `library_editor.html`, paired with its `poc_web`/`poc_api`/`poc_mobile` textareas. That page does not load `app.js`, so it cannot share the constant and is the copy a channel change forgets |
